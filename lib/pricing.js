...
// Lágmarksáskrift á mánuði, óháð pakka - án þessa myndi mjög lítið
// fyrirtæki (5-10 starfsmenn) skila nánast engum hagnaði í föstum krónum
// talið, jafnvel þó framlegðarhlutfallið líti fínt út á pappír. Þetta
// gólf tryggir að hver reikningur standi undir lágmarks yfirbyggingu
// (innheimta, stuðningur, umsjón) sama hversu fámennt fyrirtækið er.
// Um leið og pricePerEmployee x virkir starfsmenn fer yfir þessa tölu
// tekur venjulega útreikningurinn við sjálfkrafa.
const SUBSCRIPTION_MINIMUM = 4900;
...
function computeInvoice(planType, activeEmployeeCount) {
  ...
  const calculated = plan.pricePerEmployee * activeEmployeeCount;
  const subtotal = Math.max(SUBSCRIPTION_MINIMUM, calculated);
  return {
    plan,
    activeEmployeeCount,
    pricePerEmployee: plan.pricePerEmployee,
    subtotal,
    isMinimum: subtotal > calculated,
    currency: CURRENCY
  };
}

module.exports = { PLANS, SETUP_FEE, CUSTOM_HANDLING_FEE, SUBSCRIPTION_MINIMUM, CURRENCY, formatISK, computeInvoice };
