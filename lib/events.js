...
function runDailyWorkflows(companyId) {
  ...
  // lib/notifications.js itself requires daysBetween/fmt from this file.
  // Fire-and-forget: email sending shouldn't block whatever page triggered
  // this (dashboard load, etc.), and a failed/unconfigured send is already
  // handled safely inside lib/mailer.js.
  const { sendBirthdayReminders } = require('./notifications');
  sendBirthdayReminders(companyId).catch((err) => {
    console.error('Gat ekki sent áminningarpóst:', err.message);
  });
}
